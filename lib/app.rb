...
client.update('yann : salut !!!')

5.times do  #prend 5 journalistes au hasard et leur envoi un message
    journalist = arr.sample
    puts journalist
    client.update("Hello les médias #bonjour_monde #{journalist} ")
end

# liker les 25 derniers tweets avec le "#bonjour_monde"
tweets = client.search("#bonjour_monde", result_type: "recent").take(25)
client.favorite(tweets)

# suivre les 20 derniers utilisateurs qui ont le #bonjour_monde"
client.search("#bonjour_monde", result_type: "recent").take(20).each do |tweets|
client.follow(tweets.user)
end

# streamer en live pour le "#bonjour_monde"
